...
class DirectGr00tInference:
    ...
    def __init__(self, robot_config, model_path, embodiment_tag, data_config, denoising_steps=4):
        ...
        # Movement tracking for task completion detection
        self.position_history = []  # Store last 5 positions
        self.max_history = 10
        self.movement_threshold = 5.0  # Units threshold for movement detection
    ...
    def update_position_history(self, observation_dict):
        """Update position history and check if robot has stopped moving"""
        current_positions = {key: observation_dict[key] for key in self.robot_state_keys}
        ...
        # Check if we have enough history to evaluate movement
        if len(self.position_history) >= self.max_history:
    ...
    def check_task_completion(self):
        """Check if robot hasn't moved significantly over the last 5 actions"""
        if len(self.position_history) < self.max_history:
            return False
        ...
        # Check movement for each joint
        max_movement = 0.0
        for key in self.robot_state_keys:
            movement = abs(last_pos[key] - first_pos[key])
            max_movement = max(max_movement, movement)
            
        print(f"Max joint movement over last {self.max_history} actions: {max_movement:.2f} units")
        
        # If no joint moved more than threshold, task is likely complete
        if max_movement <= self.movement_threshold:
            print(f"Task completion detected! Max movement ({max_movement:.2f}) <= threshold ({self.movement_threshold})")
            return True
...
def main():
    ...
    try:
        ...
        while not STOP:
            ...
            # Check if task is complete based on movement history
            task_complete = policy_client.update_position_history(observation_dict)
            if task_complete:
                print("Task completed! Robot has stopped moving significantly.")
                STOP = True
                break
